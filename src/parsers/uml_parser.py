    ...
    def parse(uml_json):
        ...
        for link in uml_json["linkDataArray"]:
            if link["name"] == "generalization" or link["name"] == "association":
                ...
                UMLParser.increment_matrix_table_cell(matrix_classes, class_to, class_from)
            else:
                matrix_classes = 0
                asc_class = 0
                ...
                for link2 in uml_json["linkDataArray"]:
                    if len(link2["labelKeys"]) == 1 and link2["labelKeys"][0] == link_label:
                        asc_to = link2["to"]
                        asc_from = link2["from"]
                        UMLParser.increment_matrix_table_cell(matrix_classes, classes[asc_to], classes[asc_class])
                        UMLParser.increment_matrix_table_cell(matrix_classes, classes[asc_from], classes[asc_class])
                        break
