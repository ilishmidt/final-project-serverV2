    ...
    def update_sql_matrix(matrix_classes, query_classes, classes, classes_names):
        for class_name in query_classes:
            if query_classes[class_name] > 1:
                ...
                SQLParser.increment_matrix_table_cell(matrix_classes, classes[class_key], classes[class_key],
                                            query_classes[class_name] - 1)
        ...
        for class_comb in classes_combinations:
            ...
            SQLParser.increment_matrix_table_cell(matrix_classes, classes[first_class_key], classes[second_class_key], increase)
        ...
        for key in sql_json:
            if sql_json[key]['selectable']:
                ...
                if query_arr[0].lower() == 'return':
                    query_classes = SQLParser.get_classes_return_query(query_arr, classes_names)
                elif query_arr[0].lower() == 'update':
                    query = query.replace(",", ", ")
                    query = query.replace(" ,", ",")
                    query = _RE_COMBINE_WHITESPACE.sub(" ", query).strip()
                    query_arr = query.split(" ")
                    query_classes = SQLParser.get_classes_update_query(query_arr, classes_names)
                elif query_arr[0].lower() == 'insert':
                    query_classes = SQLParser.get_classes_insert_query(query_arr, classes_names)
                elif query_arr[0].lower() == 'delete':
                    query_classes = SQLParser.get_classes_delete_query(query_arr, classes_names)
                elif query_arr[0].lower() == 'connect':
                    query_classes = SQLParser.get_classes_connect_query(query_arr, classes_names)
                else:
                    continue
                SQLParser.update_sql_matrix(matrix_classes, query_classes, classes, classes_names)
                complexity = SQLParser.calculate_query_complexity(uml_json, query_classes, classes_names, query)
